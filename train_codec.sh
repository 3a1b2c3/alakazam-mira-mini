...
exec $RUN python scripts/train_codec.py \
    "${idx[@]}" \
    "${OPT_DEFAULTS[@]}" \
    run.batch_size=4 run.compile=false wandb.mode=disabled dataloader.num_workers="$WORKERS" run.log_every=50 run.checkpoint_every=3000 run.checkpoint_keep_recent=2 validation.val_every=2500 \
