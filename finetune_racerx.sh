...
exec $RUN python scripts/train_world_model.py --config-name finetune_world_model \
    ...
    run.steps="$STEPS" run.output_dir="$OUTDIR" run.checkpoint_every=7000 run.checkpoint_keep_recent=2 run.checkpoint_keep_permanent_every=-1 validation.downstream_val_every=7000 optim.scheduler.warmup_steps=200 \
